...
public class MapGraph {
										  ...
										  GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		boolean found = searchDijkstra(startNode, endNode, toExplore, visited, parentMap, nodeSearched);
		if (!found) {
			System.out.println("No path found from " + start + " to " + goal);
			return null;
		}
	...
	private static boolean searchDijkstra(MapNode startNode, MapNode endNode, PriorityQueue<MapNode> toExplore, 
...
//			System.out.println("DIJKSTRA visiting" + curr);
											 ...
											 GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		boolean found = performAStarSearch(startNode, endNode, toExplore, visited, parentMap, nodeSearched);
		if (!found) {
			System.out.println("No path found from " + start + " to " + goal);
			return null;
		}
	...
	private static boolean performAStarSearch(MapNode startNode, MapNode endNode, PriorityQueue<MapNode> toExplore, 
			HashSet<MapNode> visited, HashMap<MapNode, MapNode> parentMap, Consumer<GeographicPoint> nodeSearched) {
		startNode.setDistance(0);
